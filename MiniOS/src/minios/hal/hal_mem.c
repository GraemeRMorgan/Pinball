 ...
 * @version 
 *
 * @brief Memory part of the Hardware Abstraction Layer. 
 *
 */
#include <asf.h>
#include "hal.h"
...
*/
bool hal_mem_start( tMemoryId mem_id ){
	switch(mem_id){
		case MemSDCardFAT:	
			sdcard_fat_init = true;
			return sdcard_init(10) && fs_mount();	//SD Card + FAT
		case MemSDCard:		return false;							//unimplemented (raw access to sd card)
		case MemFlash:		return false;							//unimplemented (on board flash)
...
*/
double hal_mem_capacity( tMemoryId mem_id ){
	switch(mem_id){
		case MemSDCardFAT:	return sd_mmc_get_capacity(0) / (1024.0*1024.0); 
		case MemSDCard:		return 0;	//not implemented	
		case MemFlash:		return 0;	//not implemented	
		default:			return 0;	//error!
	}
}

/**
*	Memory FAT File Size
*
...
*/
uint32_t hal_mem_fat_file_size( uint8_t* fname ){
	FILINFO fno;
...
*/
uint32_t hal_mem_fat_file_read( uint8_t* fname, uint8_t* buffer, uint32_t buf_size ){
	FIL fil;
	FRESULT fr;
...
*/
bool hal_mem_fat_file_write( uint8_t* fname, uint8_t* buffer, uint32_t buf_size ){
	FIL fil;
...
*
*	uint8_t list[10][MAX_FILENAME_SIZE+1];
*
*	if( ( n = memory_fat_file_list( list, 10 ) ) > 0 ){
...
*/
uint32_t hal_mem_fat_file_list( uint8_t list[][MEM_FAT_MAX_FNAME_LENGTH+1], uint32_t max_num_files ){
	FILINFO fno;
	DIR dir;
	uint8_t *pc_fn;
